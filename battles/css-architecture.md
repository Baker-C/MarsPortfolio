...
- Theme layout vibes (added 2026-08-26): each theme registry entry carries a `vibe` (`horizon/grove/pond/bloom/ridge/bouquet`, exposed via `useTheme().vibe`); designs branch their page compositions on the vibe — never on theme id — so palettes stay decoupled from layouts and a new theme can adopt an existing vibe without touching the designs.
- Font customization (added 2026-08-26): typography mirrors the color system. Font tokens resolve through runtime variables (`--fd/--fb/--fs/--fh`); `src/theme/fonts.ts` defines the option pool (seven 1001fonts title fonts served from `public/fonts/` with @font-face in index.css — demo/personal-use licenses — plus matched Google Fonts) and eight font sets (each led by one title font with supporting body/label/hand fonts). `ThemeScope` writes all four font vars from the active set + per-role overrides; choices persist per design (`mars-fontset:*`, `mars-font-overrides:*`). `:root` in index.css carries the 'classic' defaults and must stay in sync with fonts.ts. The switcher's own UI uses the static `--font-ui` token (not var-routed) so the appearance drawer keeps a constant face while the site's fonts change; only its preview labels render in the fonts they name.
